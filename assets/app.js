...
// ── SHARE SYSTEM ─────────────────────────────────────────────
async function handleShareLink(path) {
  TopBar.show();
  try {
    const r = await fetch(`api.php?action=get_share_link&path=${encodeURIComponent(path)}`);
    const res = await r.json();
    if (res.success && res.link) {
      await navigator.clipboard.writeText(res.link);
      showToast('Đã copy link công khai vào clipboard!', 'success');
    } else {
      showToast('Không thể tạo link share', 'error');
    }
  } catch (e) {
    ...
    videos.forEach((v, idx) => {
      ...
      card.innerHTML = `
        <div class="thumbnail-wrapper${thumbSrc ? ' thumb-loading' : ''}">
          ...
          <button class="share-btn" data-name="${v.name}" data-path="${v.path}" title="Chia sẻ công khai">
      ...
      const shareB = card.querySelector('.share-btn');
      if (shareB) shareB.onclick = e => { e.stopPropagation(); handleShareLink(v.path); };
...
if (document.getElementById('videoPlayer')) {
  ...
  if (playerShareBtn) {
    playerShareBtn.addEventListener('click', () => handleShareLink(CURRENT_PATH));
  }
